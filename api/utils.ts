...
export function getRequestPathData(BASE_URL: string, data: string | URL) {
  ...
  const proxySearch = searchParams.toString() ? `?${searchParams.toString()}` : ''
...
export function getRequestHeaders(headers: HeadersInit) {
  ...
  const userAgent = headers instanceof Headers ? headers.get('user-agent') : null
  myHeaders.set(
    'User-Agent',
    userAgent ||
  ...
  myHeaders.set('Accept-Language', 'zh-CN,zh;q=0.9,en;q=0.8,zh-HK;q=0.7,ja;q=0.6')
  ...
  myHeaders.set('Sec-Ch-Ua', '"Google Chrome";v="131", "Chromium";v="131", "Not_A Brand";v="24"')
