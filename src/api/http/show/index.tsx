...
const getShow = async (id: string): Promise<PixivShowResponse | PixivRankErrorResponse> => {
  let result: PixivShowResponse | PixivRankErrorResponse

  try {
    const res = await request(createRequestConfig('/api/pixiv/illust', { id }))
    result = handleApiResponse(res) as PixivShowResponse | PixivRankErrorResponse
  } catch {
