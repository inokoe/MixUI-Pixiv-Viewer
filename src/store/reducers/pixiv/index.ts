...
const pixivSlice = createSlice({
  ...
  reducers: {
    ...
    setSearchParams(state, action: PayloadAction<SearchParams>) {
      ...
      const filteredWord = word?.trim()
        ? `${word} -R-18 -R18 -18+`
        : state.searchParams.word;
      const originWord = word?.trim()
        ? `${word}`
        : state.searchParams.originWord;
      state.searchParams = {
        ...state.searchParams,
        ...params,
        word: filteredWord,
    ...
    setShowHistory(state, action: PayloadAction<Illust>) {
      const currentData = Array.isArray(state.showHistory.data)
