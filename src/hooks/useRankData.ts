...
const selectRankData = createSelector(
  [(state: RootState) => state.pixiv.rank, (_: RootState, mode: string) => mode],
...
export const useRankData = (mode: string, date: string, page: number): Illust[] => {
  const location = useLocation()
  const rankInitial = useSelector((state: RootState) => state.pixiv.rankInitial)
  const rankData = useSelector((state: RootState) => selectRankData(state, mode))
  ...
  useEffect(() => {
    ...
    const hasData = rankData.some(
      item => item.date === date && item.page === page && item.illusts.length > 0
    )
    if (!hasData) {
      ;(async () => {
        MyNProgress.start()
