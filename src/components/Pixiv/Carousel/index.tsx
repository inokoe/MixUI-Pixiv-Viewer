...
const PixivCarousel = memo(
  ({ className, toggleFullscreen }: { className?: string; toggleFullscreen: () => void }) => {
    const data = useContext(DataContext)
    const imageQuality = useSelector(
      (state: RootState) => state.performance.imageLoadQuality.quality
    )
      ...
      <div className={cn('w-full h-full relative', className)}>
        <Carousel className='w-full h-full [&>div]:h-full'>
                      ...
                      <Card className='h-full w-full rounded-none border-0'>
                        <CardContent className='flex items-center justify-center p-0 h-full w-full bg-slate-100 dark:bg-zinc-800'>
                          <SkeletonImage
                            ...
                            preload={
                              index === imageSrc.length - 1 ? '' : imgSrc(imageSrc[index + 1])
                            }
                          />
                        </CardContent>
                      </Card>
          ...
          <div className='absolute inset-0 pointer-events-none'>
            <FullScreenButton
              className='absolute bottom-5 right-5 z-50 pointer-events-auto'
